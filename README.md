...
- [x] 漫天飞羽弹幕使（☀️ B站有了一个烈焰弹幕使、这里命名为：漫天飞宇）.
...
addDispose(HongApiImpl.api().get_observable_github_user(params)
        ...
        .subscribe(new Consumer<ReplyGithubUserMode>() {
            ...
            public void onError(int errorCode, String errorMsg) {
                LaLog.e("hong----请求数据失败-服务器异常other:errorMsg=\n" + errorMsg + "errorCode=" + errorMsg);
            }
        }));
```
...
**⚠️注意**
> * 下载是一个耗时的过程，但本示例的下载已经默认交给livery的后台线程执行了，这里不需要再单独去开启一个线程.
...
```
为了增加开发者控制每一条弹幕（即使Barrage属性里面有值的情况），提供以下操作
```java
mBarrageAdapter.addBarrage(barrage, new OnBarrageLayout() {
    @Override
    public View barrageLayout(View layoutView, Barrage barrage) {
        ...
        return barrageItem.getConverView();
    }
});
```
...
### 3.DEMO演示
部分`demo_livery1.1.11.apk`效果图，包含了livery情景能力，**在我的知乎可以获取更多实用的技巧.**
